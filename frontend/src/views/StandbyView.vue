...
onMounted(async () => {
    ...
    echo.channel('drivers').listen('TripCreated', (event) => {
        ...
        // set similar values from event into trip store
        trip.$patch(event.trip)
        // set timeout as upon render the gMap value is null
...
const handleDeclineTrip = () => {
    trip.reset() // using reset method from trip store instead of patching values manually
    title.value = "Waiting for ride request..." // reseting title ref
}

const initMapDirections = () => {
    // logic similar to MapView component, but using trip
    gMap.value.$mapPromise.then((mapObject) => {
        let originPoint = new google.maps.LatLng(trip.origin ),
            ...
            // creating route between driver and rider
